...
def test_format_primary_metric_long():
    ...
    formatted = format_primary_metric(
        evaluation, "profit_ratio", ["adjusted_probability", "expected_return"]
    )
    ...
    assert formatted.get("profit_ratio_formatted") == formatted_str, (
        "Long metric formatting failed"
    )
    logging.info(
        "Long metric formatting passed with output: %s",
        formatted.get("profit_ratio_formatted"),
    )
...
def test_format_primary_metric_spread():
    ...
    expected_weight = expected_score / total if total > 0 else 0
    formatted_str = (
        f"{1.98765:.2f} (Score: {expected_score:.2f}, Weight: {expected_weight:.2f})"
    )
    assert formatted.get("risk_reward_ratio_formatted") == formatted_str, (
        "Spread metric formatting failed"
    )
    logging.info(
