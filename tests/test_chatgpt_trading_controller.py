...
def test_gpt_response_triggers_trades(monkeypatch):
    response = json.dumps(
        {
            "actions": [{"action": "buy", "symbol": "MSFT", "quantity": 2}],
            "request": "done",
        }
    )
    monkeypatch.setattr(
        "fundrunner.bots.chatgpt_trading_controller.ask_gpt", lambda prompt: response
    )
    monkeypatch.setattr(
        "fundrunner.bots.chatgpt_trading_controller.PortfolioManager", lambda: DummyPM()
    )
    dummy = DummyTM()
    monkeypatch.setattr(
        "fundrunner.bots.chatgpt_trading_controller.TradeManager", lambda: dummy
    )
